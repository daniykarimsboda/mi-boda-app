...
const sd    = (data?.categories?.length) ? data : INIT;
  const spent = sd.categories.reduce((a,c) => a+c.budgetReal, 0);
  const estim = sd.categories.reduce((a,c) => a+c.budgetEstimated, 0);
  const d     = daysUntil(sd.weddingDate);
  const cats  = sd.categories.map(c => ({
  ...
  const exportData = () => {
    const lines = [
      `💍 ${safesd.coupleName} — Wedding OS`,
      `Fecha: ${data.weddingDate} | Días: ${d}`, ``,
      `PRESUPUESTO: ${fmt(safesd.budget)} | Gastado: ${fmt(spent)}`, ``,
      `CATEGORÍAS:`,
      ...safesd.categories.map(c=>`• ${c.label}: ${c.tasks.filter(t=>t.done).length}/${c.tasks.length} | ${fmt(c.budgetReal)}`), ``,
      `INVITADOS: ${safesd.guests.length}`,
      `Confirmados: ${safesd.guests.filter(g=>g.rsvp==="confirmado").length}`,
      `Pendientes: ${safesd.guests.filter(g=>g.rsvp==="pendiente").length}`, ``,
      `TIMELINE:`,
      ...safesd.timeline.map(t=>`  ${t.time}  ${t.activity}`),
    ].join("\n");
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([lines],{type:"text/plain"}));
    a.download = `wedding-os-${safesd.coupleName.replace(/\s/g,"_")}.txt`;
    a.click();
  };
        ...
        <div style={{padding:"22px 18px 16px",borderBottom:"1px solid rgba(224,187,228,.2)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          {sidebarOpen && (
            <div>
              ...
              <div className="serif" style={{fontSize:22,color:"#B2AC88",fontStyle:"italic"}}>OS ✦</div>
            </div>
          )}
          <button onClick={()=>setSO(o=>!o)} style={{background:"none",border:"none",cursor:"pointer",color:"#9b8ab4",padding:4}}>
            {sidebarOpen ? <X size={18}/> : <Menu size={18}/>}
          </button>
        </div>
        {sidebarOpen && (
          <div style={{padding:"12px 18px",borderBottom:"1px solid rgba(224,187,228,.15)"}}>
            <div className="serif" style={{fontSize:16,color:"#6b5c7e",fontStyle:"italic"}}>{safesd.coupleName}</div>
          ...
          <div className="fade">
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:28,flexWrap:"wrap",gap:12}}>
              <div>
                <div className="serif" style={{fontSize:38,color:"#4a3a5c",fontWeight:300,lineHeight:1.15}}>
                  Bienvenida, <span style={{fontStyle:"italic",color:"#B2AC88"}}>{safesd.coupleName.split("&")[0].trim()} ✦</span>
                </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(215px,1fr))",gap:16,marginBottom:24}}>
              ...
              <div className="glass" style={{borderRadius:22,padding:"22px 24px",position:"relative",overflow:"hidden"}}>
                ...
                {editD ? (
                  ...
                  <button onClick={()=>setEditD(true)} style={{marginTop:10,display:"flex",alignItems:"center",gap:5,background:"none",border:"none",cursor:"pointer",color:"#c4a0cc",fontSize:12}}>
              ...
              <div className="glass" style={{borderRadius:22,padding:"22px 24px"}}>
                ...
                <div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:16}}>
                  <div className="serif" style={{fontSize:28,color:"#4a3a5c"}}>{fmt(safesd.budget)}</div>
                  <button onClick={()=>{const v=prompt("Nuevo presupuesto MXN:",safesd.budget);if(v&&!isNaN(v))upd(x=>{x.budget=+v;});}}
                    style={{background:"none",border:"none",cursor:"pointer",color:"#ccc"}}><Edit2 size={13}/></button>
                </div>
                {[{l:"Gastado",v:spent,c:"#c77daa"},{l:"Estimado",v:estim,c:"#E0BBE4"},{l:"Disponible",v:safesafesd.budget-spent,c:"#B2AC88"}].map(r=>(
                  <div key={r.l} style={{marginBottom:8}}>
                    ...
                    <div style={{height:5,borderRadius:3,background:"#f0e8f5"}}>
                      <div style={{height:"100%",borderRadius:3,background:r.c,width:`${Math.min(100,(r.v/safesafesd.budget)*100)}%`,transition:"width .8s"}}/>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="glass" style={{borderRadius:22,padding:"22px 24px"}}>
                ...
                <div className="serif" style={{fontSize:44,color:"#4a3a5c",fontWeight:300,marginBottom:14}}>{safesd.guests.length}</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                  {[{l:"Conf.",v:safesd.guests.filter(g=>g.rsvp==="confirmado").length,c:"#B2AC88"},
                    {l:"Pend.",v:safesd.guests.filter(g=>g.rsvp==="pendiente").length,c:"#FFD580"},
                    {l:"Rech.",v:safesd.guests.filter(g=>g.rsvp==="rechazado").length,c:"#F4A5A5"}].map(s=>(
              ...
              <div className="glass" style={{borderRadius:22,padding:"22px 24px"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:11,color:"#aaa",marginTop:2}}>wedding_state · id={ROW_ID}</div>
                  </div>
                </div>
                <div style={{fontSize:12,color:"#aaa",lineHeight:1.65,marginBottom:12}}>
                  Cambios sincronizados automáticamente. Abre en otro dispositivo para colaborar.
                </div>
                <SyncBadge status={sync}/>
              </div>
            </div>
            ...
            <div className="glass" style={{borderRadius:22,padding:"26px 30px",marginBottom:22}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:28,justifyContent:"space-around"}}>
                {cats.map((c,i) => <Ring key={c.id} pct={c.pct} color={COLORS[i]} sz={92} label={c.label} sub={`${c.tasks.filter(t=>t.done).length}/${c.tasks.length}`}/>)}
              </div>
            </div>
            ...
            <div className="glass" style={{borderRadius:22,padding:"26px 30px"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {safesd.categories.flatMap(c=>c.tasks.filter(t=>!t.done).map(t=>({...t,cat:c.label,cc:c.color})))
                  ...
                  <div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",borderRadius:13,background:"rgba(255,255,255,.55)",border:"1px solid rgba(224,187,228,.2)"}}>
                    ...
                    <span style={{fontSize:10,padding:"3px 9px",borderRadius:20,
                      ...
                      color:t.priority==="alta"?"#c05a5a":t.priority==="media"?"#8a7230":"#5a6a45"}}>{t.priority}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              {cats.map((cat) => {
                ...
                return (
                  <div key={cat.id} className="glass" style={{borderRadius:20,overflow:"hidden"}}>
                    <button onClick={()=>setExCat(open?null:cat.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:14,padding:"18px 22px",background:"none",border:"none",cursor:"pointer",textAlign:"left"}}>
                                ...
                                <div style={{flex:1,minWidth:0}}>
                                  ...
                                  <div style={{display:"flex",gap:8,marginTop:4,flexWrap:"wrap"}}>
                                    ...
                                    <span style={{fontSize:10,padding:"1px 7px",borderRadius:20,
                                      ...
                                      color:tk.priority==="alta"?"#c05a5a":tk.priority==="media"?"#8a7230":"#5a6a45"}}>{tk.priority}</span>
                                  </div>
                                </div>
                                <button onClick={()=>upd(x=>{const c=x.categories.find(c=>c.id===cat.id);c.tasks=c.tasks.filter(t=>t.id!==tk.id);})}
                                  style={{background:"none",border:"none",cursor:"pointer",color:"#ddd",padding:2,flexShrink:0}}><Trash2 size={13}/></button>
                              </div>
                            ))}
                            ...
                            {[["budgetEstimated","Presupuesto Estimado"],["budgetReal","Gasto Real"]].map(([f,l])=>(
                              <div key={f} style={{marginBottom:10}}>
                                ...
                                <div style={{display:"flex",alignItems:"center",gap:6,padding:"8px 12px",borderRadius:10,border:"1px solid rgba(224,187,228,.3)",background:"white"}}>
                                  ...
                                  <input type="number" value={cat[f]} onChange={e=>upd(x=>{x.categories.find(c=>c.id===cat.id)[f]=+e.target.value;})}
          ...
          <div className="fade">
            ...
            <div className="glass" style={{borderRadius:16,padding:"14px 20px",marginBottom:14,display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {["todos","confirmado","pendiente","rechazado"].map(f=>(
                  <button key={f} onClick={()=>setGf(f)} style={{padding:"5px 13px",borderRadius:20,border:"1px solid",fontSize:12,cursor:"pointer",
                    borderColor:gf===f?"#E0BBE4":"transparent",background:gf===f?"#E0BBE420":"transparent",color:gf===f?"#7b4f8a":"#aaa"}}>{f}</button>
                ))}
              </div>
            </div>
            <div className="glass" style={{borderRadius:20,overflow:"hidden"}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                ...
                <tbody>
                  {safesd.guests.filter(g=>(gf==="todos"||g.rsvp===gf)&&g.name.toLowerCase().includes(gs.toLowerCase())).map((g,i)=>(
                    <tr key={g.id} style={{borderBottom:"1px solid rgba(224,187,228,.1)",background:i%2===0?"transparent":"rgba(255,255,255,.35)"}}>
                      ...
                      <td style={{padding:"12px 16px"}}><input value={g.allergies} onChange={e=>upd(x=>{x.guests.find(y=>y.id===g.id).allergies=e.target.value;})}
                        style={{padding:"5px 10px",borderRadius:8,border:"1px solid #eee",background:"white",fontSize:13,width:130,color:"#555"}}/></td>
                      <td style={{padding:"12px 16px"}}><input value={g.table} onChange={e=>upd(x=>{x.guests.find(y=>y.id===g.id).table=e.target.value;})}
                        style={{padding:"5px 10px",borderRadius:8,border:"1px solid #eee",background:"white",fontSize:13,width:60,textAlign:"center",color:"#555"}}/></td>
                      <td style={{padding:"12px 16px"}}><button onClick={()=>upd(x=>{x.guests=x.guests.filter(y=>y.id!==g.id);})}
                        style={{background:"none",border:"none",cursor:"pointer",color:"#ddd"}}><Trash2 size={14}/></button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                return (
                  <div key={item.id} style={{display:"flex",gap:20,marginBottom:14,alignItems:"flex-start"}}>
                    ...
                    <div className="glass" style={{flex:1,borderRadius:14,padding:"12px 16px",display:"flex",gap:12,alignItems:"center"}}>
                      <input value={item.time} onChange={e=>upd(x=>{x.timeline.find(t=>t.id===item.id).time=e.target.value;})}
                      ...
                      <input value={item.activity} onChange={e=>upd(x=>{x.timeline.find(t=>t.id===item.id).activity=e.target.value;})}
                      ...
                      <button onClick={()=>upd(x=>{x.timeline=x.timeline.filter(t=>t.id!==item.id);})}
                        style={{background:"none",border:"none",cursor:"pointer",color:"#ddd",flexShrink:0}}><Trash2 size={13}/></button>
                    </div>
                  </div>
                );
              })}
...
function VisionForm({ catId, upd, color }) {
  ...
  if(!open)return<button onClick={()=>setOpen(true)} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 13px",borderRadius:11,border:`1px dashed ${color}`,background:"transparent",cursor:"pointer",color:"#bbb",fontSize:13,width:"100%"}}><Plus size={14}/> Agregar imagen / Pinterest URL</button>;
  return<div style={{padding:13,borderRadius:12,background:"rgba(255,255,255,0.75)",border:"1px solid rgba(224,187,228,0.3)",display:"flex",flexDirection:"column",gap:9}}>
    <input placeholder="URL de imagen..." value={url} onChange={e=>setUrl(e.target.value)} style={{padding:"8px 11px",borderRadius:8,border:"1px solid #eee",fontSize:13}}/>
    <input placeholder="Etiqueta" value={label} onChange={e=>setLabel(e.target.value)} style={{padding:"8px 11px",borderRadius:8,border:"1px solid #eee",fontSize:13}}/>
    <div style={{display:"flex",gap:8}}>
...
function TimelineForm({ upd }) {
  ...
  return<div className="glass" style={{flex:1,borderRadius:14,padding:"11px 16px",display:"flex",gap:10,alignItems:"center"}}>
    <input type="time" value={time} onChange={e=>setTime(e.target.value)} style={{width:82,padding:"6px 9px",borderRadius:8,border:"1px solid rgba(224,187,228,.35)",fontSize:13,color:"#7b4f8a"}}/>
    <input placeholder="Nueva actividad..." value={activity} onChange={e=>setActivity(e.target.value)} style={{flex:1,padding:"6px 9px",borderRadius:8,border:"1px solid rgba(224,187,228,.2)",background:"transparent",fontSize:14,color:"#4a3a5c"}}/>
    <button onClick={ok} style={{padding:"7px 15px",borderRadius:9,background:"#E0BBE4",border:"none",cursor:"pointer",color:"white",display:"flex",alignItems:"center"}}><Plus size={14}/></button>
  </div>;
}
