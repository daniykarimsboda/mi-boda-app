...
import {
  ...
  Cloud, CloudOff, Loader2, DollarSign, RefreshCw, Image, StickyNote
} from "lucide-react";
...
const INIT = {
  ...
  categories: [
    ...
    { id:"musica", icon:"Music", label:"Música", color:"#B3E5FC", budgetEstimated:25000, budgetReal:0,
      tasks:[], visionItems:[]},
    { id:"vestuario", icon:"Shirt", label:"Vestuario", color:"#FFE0B2", budgetEstimated:35000, budgetReal:0,
      tasks:[], visionItems:[]},
    { id:"logistica", icon:"Car", label:"Logística", color:"#F8BBD9", budgetEstimated:20000, budgetReal:0,
      tasks:[], visionItems:[]},
    { id:"foto", icon:"Camera", label:"Fotografía", color:"#E1BEE7", budgetEstimated:30000, budgetReal:0,
      tasks:[], visionItems:[]},
  ],
  timeline:[
    {id:"tl1",time:"07:00",activity:"Despertar",icon:"Star"},
...
function SyncBadge({ status }) {
  ...
  return <span style={{display:"inline-flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:20,background:m.bg,border:`1px solid ${m.bd}`,color:m.c,fontSize:12,fontWeight:500,whiteSpace:"nowrap"}}>{m.icon} {m.text}</span>;
}
...
function Ring({ pct, color, sz=88, label, sub }) {
  ...
  return <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
    ...
    <div style={{textAlign:"center"}}><div style={{fontSize:12,fontWeight:500,color:"#6b6b8a"}}>{label}</div><div style={{fontSize:11,color:"#aaa"}}>{sub}</div></div>
  </div>;
}
...
// Componente para galería dentro de categoría
...
// Componente para el collage dinámico en Dashboard
...
export default function App() {
  ...
  const [guestStats, setGuestStats] = useState({ total:0, confirmados:0, pendientes:0 });
  ...
  const loadGuestStats = async () => {
    ...
    if (guests) setGuestStats({ total: guests.length, confirmados: guests.filter(g=>g.rsvp===true).length, pendientes: guests.filter(g=>g.rsvp===false).length });
  };
  ...
  const exportData = () => {
    const lines = [`💍 ${data.coupleName} — Wedding OS`, `Fecha: ${data.weddingDate} | Días: ${d}`, `PRESUPUESTO: ${fmt(data.budget)} | Gastado: ${fmt(spent)}`, "CATEGORÍAS:", ...data.categories.map(c=>`• ${c.label}: ${c.tasks.filter(t=>t.done).length}/${c.tasks.length} | ${fmt(c.budgetReal)}`), "INVITADOS:", `Confirmados: ${guestStats.confirmados}`, `Pendientes: ${guestStats.pendientes}`, "TIMELINE:", ...data.timeline.map(t=>`  ${t.time}  ${t.activity}`)];
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:"linear-gradient(135deg,#F5F0F8 0%,#EFF5F0 50%,#F8F2ED 100%)",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:"32px 28px"}}>
        {tab === "dashboard" && (
          <div className="fade">
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:28}}><div><div className="serif" style={{fontSize:38,color:"#4a3a5c",fontWeight:300}}>Bienvenida, <span style={{fontStyle:"italic",color:"#B2AC88"}}>{data.coupleName.split("&")[0].trim()} ✦</span></div><div style={{fontSize:15,color:"#aaa"}}>Todo en un solo lugar</div></div><SyncBadge status={sync}/></div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(215px,1fr))",gap:16,marginBottom:24}}>
              <div className="glass" style={{borderRadius:22,padding:"22px 24px"}}><div style={{fontSize:11,fontWeight:500,color:"#B2AC88",marginBottom:10}}>Cuenta Regresiva</div><div className="serif" style={{fontSize:56,color:"#4a3a5c",fontWeight:300}}>{d}</div><div style={{fontSize:13,color:"#bbb"}}>días restantes</div>{editD?<div style={{marginTop:12,display:"flex",gap:8}}><input type="date" defaultValue={data.weddingDate} id="di"/><button onClick={()=>{upd(x=>{x.weddingDate=document.getElementById("di").value;});setEditD(false);}}><Save size={14}/></button></div>:<button onClick={()=>setEditD(true)} style={{marginTop:10}}><Edit2 size={12}/> {data.weddingDate}</button>}</div>
              <div className="glass" style={{borderRadius:22,padding:"22px 24px"}}><div style={{fontSize:11,fontWeight:500,color:"#B2AC88",marginBottom:10}}>Presupuesto Total</div><div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:16}}><div className="serif" style={{fontSize:28,color:"#4a3a5c"}}>{fmt(data.budget)}</div><button onClick={()=>{const v=prompt("Nuevo presupuesto:",data.budget);if(v)upd(x=>{x.budget=+v;});}}><Edit2 size={13}/></button></div>{[{l:"Gastado",v:spent,c:"#c77daa"},{l:"Estimado",v:estim,c:"#E0BBE4"},{l:"Disponible",v:data.budget-spent,c:"#B2AC88"}].map(r=><div key={r.l} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12}}><span>{r.l}</span><span>{fmt(r.v)}</span></div><div style={{height:5,background:"#f0e8f5"}}><div style={{height:"100%",background:r.c,width:`${Math.min(100,(r.v/data.budget)*100)}%`}}/></div></div>)}</div>
              <div className="glass" style={{borderRadius:22,padding:"22px 24px"}}><div style={{fontSize:11,fontWeight:500,color:"#B2AC88",marginBottom:10}}>Invitados</div><div className="serif" style={{fontSize:44,color:"#4a3a5c",fontWeight:300}}>{guestStats.total}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><div><div>{guestStats.confirmados}</div><div>Conf.</div></div><div><div>{guestStats.pendientes}</div><div>Pend.</div></div></div></div>
              <div className="glass" style={{borderRadius:22,padding:"22px 24px"}}><div style={{fontSize:11,fontWeight:500,color:"#B2AC88",marginBottom:12}}>Colaboración ☁</div><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:44,height:44,borderRadius:14,background:"rgba(224,187,228,.25)",display:"flex",alignItems:"center",justifyContent:"center"}}>{sync==="error"?<CloudOff size={22}/>:<Cloud size={22}/>}</div><div><div style={{fontSize:13,fontWeight:600,color:"#4a3a5c"}}>Supabase</div><div style={{fontSize:11,color:"#aaa"}}>Realtime sync</div></div></div></div>
            </div>
            ...
            <div className="glass" style={{borderRadius:22,padding:"26px 30px"}}><div className="serif" style={{fontSize:24,color:"#4a3a5c",marginBottom:18}}>Próximas Tareas ✦</div>{data.categories.flatMap(c=>c.tasks.filter(t=>!t.done).map(t=>({...t,cat:c.label,cc:c.color}))).sort((a,b)=>new Date(a.date)-new Date(b.date)).slice(0,5).map(t=><div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",borderRadius:13,background:"rgba(255,255,255,.55)",border:"1px solid rgba(224,187,228,.2)"}}><div style={{width:10,height:10,borderRadius:"50%",background:t.cc}}/><div><div>{t.text}</div><div style={{fontSize:11,color:"#bbb"}}>{t.cat} · {t.date}</div></div><span style={{fontSize:10,padding:"3px 9px",borderRadius:20,background:t.priority==="alta"?"#F4A5A530":t.priority==="media"?"#FFD58030":"#B2AC8830"}}>{t.priority}</span></div>)}</div>
            <VisionBoardCollage categories={data.categories} />
          </div>
        )}
        {tab === "categorias" && (
          <div className="fade">
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:26}}><div className="serif" style={{fontSize:38,color:"#4a3a5c",fontWeight:300}}>Categorías <span style={{fontStyle:"italic",color:"#B2AC88"}}>✦</span></div><SyncBadge status={sync}/></div>
            <div className="flex justify-end mb-4"><button onClick={()=>setShowNewCatModal(true)} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#E0BBE4]/30 border border-[#E0BBE4]/50"><Plus size={16}/> Agregar Categoría</button></div>
              ...
              {cats.map(cat=>{
                ...
                return <div key={cat.id} className="glass" style={{borderRadius:20,overflow:"hidden"}}>
                  <div className="flex items-center justify-between w-full">
                    <button onClick={()=>setExCat(open?null:cat.id)} className="flex flex-1 items-center gap-4 p-4">
                      <div style={{width:42,height:42,borderRadius:13,background:cat.color+"55",display:"flex",alignItems:"center",justifyContent:"center"}}><CI size={20} color={cat.color}/></div>
                      <div style={{flex:1}}><div style={{fontSize:16,fontWeight:500,color:"#4a3a5c"}}>{cat.label}</div><div style={{display:"flex",gap:10,alignItems:"center",marginTop:5}}><div style={{width:90,height:5,background:"#f0e8f5"}}><div style={{height:"100%",background:cat.color,width:`${cat.pct}%`}}/></div><span style={{fontSize:12,color:"#bbb"}}>{cat.pct}%</span></div></div>
                      <div><div style={{textAlign:"right"}}><div>{fmt(cat.budgetReal)}</div><div style={{fontSize:11,color:diff>=0?"#7a9a6a":"#c05a5a"}}>{diff>=0?"↓":"↑"} {fmt(Math.abs(diff))}</div></div>{open?<ChevronUp size={18}/>:<ChevronDown size={18}/>}</div>
                    </button>
                  ...
                  {open && <div style={{padding:"4px 22px 24px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
                    <div><div style={{fontSize:12,fontWeight:500,color:"#9b8ab4",marginBottom:12}}>Checklist</div><div style={{display:"flex",flexDirection:"column",gap:8}}>
                      {cat.tasks.map(tk=>(
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:16}}>
                      <div style={{padding:16,borderRadius:14,background:"rgba(255,255,255,.65)"}}><div style={{fontSize:12,fontWeight:500,color:"#9b8ab4",marginBottom:12}}>Finanzas</div><div><div className="text-xs text-[#aaa]">Presupuesto Estimado</div><div className="flex items-center gap-2 p-2 rounded-lg bg-white"><span>$</span><input type="number" value={cat.budgetEstimated} onChange={e=>upd(x=>{x.categories.find(c=>c.id===cat.id).budgetEstimated=+e.target.value;})} className="flex-1 bg-transparent"/></div></div><div><div className="text-xs text-[#aaa]">Gasto Real</div><div className="p-2 rounded-lg bg-[#E0BBE4]/10">{fmt(cat.budgetReal)}</div></div><div className="p-2 rounded-lg bg-[#B2AC88]/10"><div className="text-xs text-[#aaa]">Diferencia</div><div className={`font-semibold ${diff>=0?"text-[#5a7a4a]":"text-[#c05a5a]"}`}>{diff>=0?"+":""}{fmt(diff)}</div></div></div>
                      <div><div style={{fontSize:12,fontWeight:500,color:"#9b8ab4",marginBottom:12}}>Vision Board</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{cat.visionItems.map(v=><div key={v.id} className="relative"><img src={v.url} alt="" className="w-full h-24 object-cover rounded-lg" onError={e=>e.target.style.display="none"}/><button onClick={()=>upd(x=>{const c=x.categories.find(c=>c.id===cat.id);c.visionItems=c.visionItems.filter(i=>i.id!==v.id);})} className="absolute top-1 right-1 bg-black/50 rounded-full p-0.5"><X size={12} color="white"/></button></div>)}</div><VisionForm catId={cat.id} upd={upd} color={cat.color}/></div>
                      <div><div style={{fontSize:12,fontWeight:500,color:"#9b8ab4",marginBottom:12}}>Galería</div><CategoryGallery catId={cat.id} gallery={cat.gallery} upd={upd} /></div>
                    </div>
                  </div>}
                </div>;
              })}
        ...
        {tab === "invitados" && <div><div style={{display:"flex",justifyContent:"space-between",marginBottom:26}}><div className="serif" style={{fontSize:38,color:"#4a3a5c",fontWeight:300}}>Invitados ✦</div><SyncBadge status={sync}/></div><ErrorBoundary><Suspense><GuestManager/></Suspense></ErrorBoundary></div>}
        {tab === "mesas" && <div><div style={{display:"flex",justifyContent:"space-between",marginBottom:26}}><div className="serif" style={{fontSize:38,color:"#4a3a5c",fontWeight:300}}>Mesas ✦</div><SyncBadge status={sync}/></div><div className="glass rounded-2xl p-6 space-y-6"><TableConfig/><ErrorBoundary><Suspense><TableDashboard/></Suspense></ErrorBoundary></div></div>}
        {tab === "finanzas" && <div><div style={{display:"flex",justifyContent:"space-between",marginBottom:26}}><div className="serif" style={{fontSize:38,color:"#4a3a5c",fontWeight:300}}>Finanzas ✦</div><SyncBadge status={sync}/></div><FinancialBreakdown categories={data.categories} onPaymentAdded={()=>{upd(async x=>await recalcBudgetRealFromPayments(x));}}/></div>}
        {tab === "timeline" && <div><div style={{display:"flex",justifyContent:"space-between",marginBottom:26}}><div className="serif" style={{fontSize:38,color:"#4a3a5c",fontWeight:300}}>Día B ✦</div><SyncBadge status={sync}/></div><div style={{maxWidth:640,margin:"0 auto"}}><div style={{position:"relative",paddingLeft:30}}><div style={{position:"absolute",left:12,top:0,bottom:0,width:2,background:"#E0BBE4"}}/>{data.timeline.map((item,i)=><div key={item.id} style={{display:"flex",gap:12,marginBottom:14}}><div style={{width:40,height:40,borderRadius:"50%",background:i%2===0?"#E0BBE4":"#B2AC88",display:"flex",alignItems:"center",justifyContent:"center"}}>{IconMap[item.icon]?IconMap[item.icon]({size:17,color:"white"}):<Star size={17} color="white"/>}</div><div className="glass flex-1 p-2 rounded-xl flex gap-2"><input value={item.time} onChange={e=>upd(x=>{x.timeline.find(t=>t.id===item.id).time=e.target.value})} className="w-16 p-1 rounded"/><input value={item.activity} onChange={e=>upd(x=>{x.timeline.find(t=>t.id===item.id).activity=e.target.value})} className="flex-1 p-1 rounded"/><button onClick={()=>upd(x=>{x.timeline=x.timeline.filter(t=>t.id!==item.id)})}><Trash2 size={14}/></button></div></div>)}</div><TimelineForm upd={upd}/></div></div>}
        {tab === "postits" && <div><div style={{display:"flex",justifyContent:"space-between",marginBottom:26}}><div className="serif" style={{fontSize:38,color:"#4a3a5c",fontWeight:300}}>Post-its ✦</div><SyncBadge status={sync}/></div><PostItBoard /></div>}
      </div>

      {/* Modales */}
      {showNewCatModal && <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50"><div className="bg-white rounded-2xl p-6 max-w-md"><h3 className="serif text-2xl mb-4">Nueva Categoría</h3><div className="space-y-3"><input type="text" placeholder="Nombre" value={newCatName} onChange={e=>setNewCatName(e.target.value)} className="w-full p-2 border rounded"/><div><label>Icono</label><div className="flex flex-wrap gap-2 mt-1">{["Heart","Users","Sparkles","Flower","Music","Shirt","UtensilsCrossed","Camera","Car","Star","Gift"].map(icon=>{const Ic=IconMap[icon];return <button key={icon} onClick={()=>setNewCatIcon(icon)} className={`p-2 rounded ${newCatIcon===icon?'bg-[#E0BBE4]':'border'}`}><Ic size={20}/></button>;})}</div></div><input type="number" placeholder="Presupuesto estimado" value={newCatBudget} onChange={e=>setNewCatBudget(e.target.value)} className="w-full p-2 border rounded"/></div><div className="flex justify-end gap-3 mt-6"><button onClick={()=>setShowNewCatModal(false)} className="px-4 py-2 rounded-full bg-gray-200">Cancelar</button><button onClick={createNewCategory} className="px-4 py-2 rounded-full bg-[#E0BBE4] text-white">Crear</button></div></div></div>}
      {editCatModal && <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50"><div className="bg-white rounded-2xl p-6 max-w-md"><h3 className="serif text-2xl mb-4">Editar Categoría</h3><div className="space-y-3"><input type="text" placeholder="Nombre" value={editCatName} onChange={e=>setEditCatName(e.target.value)} className="w-full p-2 border rounded"/><div><label>Icono</label><div className="flex flex-wrap gap-2 mt-1">{["Heart","Users","Sparkles","Flower","Music","Shirt","UtensilsCrossed","Camera","Car","Star","Gift"].map(icon=>{const Ic=IconMap[icon];return <button key={icon} onClick={()=>setEditCatIcon(icon)} className={`p-2 rounded ${editCatIcon===icon?'bg-[#E0BBE4]':'border'}`}><Ic size={20}/></button>;})}</div></div><input type="number" placeholder="Presupuesto estimado" value={editCatBudget} onChange={e=>setEditCatBudget(e.target.value)} className="w-full p-2 border rounded"/></div><div className="flex justify-end gap-3 mt-6"><button onClick={()=>setEditCatModal(null)} className="px-4 py-2 rounded-full bg-gray-200">Cancelar</button><button onClick={saveEditCategory} className="px-4 py-2 rounded-full bg-[#E0BBE4] text-white">Guardar</button></div></div></div>}
      {editTaskModal && <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50"><div className="bg-white rounded-2xl p-6 max-w-md"><h3 className="serif text-2xl mb-4">Editar Tarea</h3><div className="space-y-3"><input type="text" placeholder="Descripción" value={editTaskText} onChange={e=>setEditTaskText(e.target.value)} className="w-full p-2 border rounded"/><input type="date" value={editTaskDate} onChange={e=>setEditTaskDate(e.target.value)} className="w-full p-2 border rounded"/><select value={editTaskPriority} onChange={e=>setEditTaskPriority(e.target.value)} className="w-full p-2 border rounded"><option value="alta">Alta</option><option value="media">Media</option><option value="baja">Baja</option></select></div><div className="flex justify-end gap-3 mt-6"><button onClick={()=>setEditTaskModal(null)} className="px-4 py-2 rounded-full bg-gray-200">Cancelar</button><button onClick={saveEditTask} className="px-4 py-2 rounded-full bg-[#E0BBE4] text-white">Guardar</button></div></div></div>}
    </div>
  );
}
...
function TaskForm({catId,upd,color}) {
  ...
  if(!open) return <button onClick={()=>setOpen(true)} className="flex items-center gap-1 text-sm px-3 py-1 rounded-full bg-white/40"><Plus size={14}/> Agregar tarea</button>;
  return <div className="p-3 rounded-xl bg-white/50 border"><input placeholder="Descripción" value={text} onChange={e=>setText(e.target.value)} className="w-full p-1 rounded border"/><div className="flex gap-2 mt-2"><input type="date" value={date} onChange={e=>setDate(e.target.value)} className="flex-1 p-1 rounded border"/><select value={prio} onChange={e=>setPrio(e.target.value)} className="p-1 rounded border"><option value="alta">Alta</option><option value="media">Media</option><option value="baja">Baja</option></select></div><div className="flex justify-end gap-2 mt-2"><button onClick={ok} className="px-3 py-1 rounded-full bg-[#E0BBE4] text-white">Guardar</button><button onClick={()=>setOpen(false)} className="px-3 py-1 rounded-full bg-gray-200">Cancelar</button></div></div>;
}
function VisionForm({catId,upd,color}) {
  ...
  if(!open) return <button onClick={()=>setOpen(true)} className="flex items-center gap-1 text-sm px-3 py-1 rounded-full bg-white/40 w-full mt-2"><Plus size={14}/> Agregar imagen</button>;
  return <div className="p-3 rounded-xl bg-white/50 border mt-2"><input placeholder="URL imagen" value={url} onChange={e=>setUrl(e.target.value)} className="w-full p-1 rounded border"/><input placeholder="Etiqueta" value={label} onChange={e=>setLabel(e.target.value)} className="w-full p-1 rounded border mt-1"/><div className="flex justify-end gap-2 mt-2"><button onClick={ok} className="px-3 py-1 rounded-full bg-[#E0BBE4] text-white">Agregar</button><button onClick={()=>setOpen(false)} className="px-3 py-1 rounded-full bg-gray-200">Cancelar</button></div></div>;
}
function TimelineForm({upd}) {
  ...
  return <div className="glass flex gap-2 p-2 rounded-xl mt-4"><input type="time" value={time} onChange={e=>setTime(e.target.value)} className="w-24 p-1 rounded"/><input placeholder="Nueva actividad..." value={activity} onChange={e=>setActivity(e.target.value)} className="flex-1 p-1 rounded"/><button onClick={ok} className="p-1 rounded-full bg-[#E0BBE4] text-white"><Plus size={16}/></button></div>;
}
