...
export default function WeddingOS() {
  ...
  const exportData = () => {
    const txt = [
      ...
      `━━ PRESUPUESTO ━━`,
      `Total: ${fmt(data.budget)}  |  Gastado: ${fmt(spent)}  |  Estimado: ${fmt(estim)}`, ``,
      `━━ CATEGORÍAS ━━`,
      ...data.categories.map(c=>`• ${c.label}: ${c.tasks.filter(t=>t.done).length}/${c.tasks.length} tareas  |  ${fmt(c.budgetReal)} gastado`), ``,
      `━━ INVITADOS (${data.guests.length}) ━━`,
      `Confirmados: ${data.guests.filter(g=>g.rsvp==="confirmado").length}  |  Pendientes: ${data.guests.filter(g=>g.rsvp==="pendiente").length}  |  Rechazados: ${data.guests.filter(g=>g.rsvp==="rechazado").length}`, ``,
      `━━ TIMELINE ━━`,
