    ...
    <div className="glass rounded-2xl p-6">
      <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowModal(true)} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#E0BBE4]/30 border border-[#E0BBE4]/50 text-[#7b4f8a] hover:bg-[#E0BBE4]/50 transition"><Plus size={16}/> Agregar pago</button>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {payments.map(p => {
                ...
                const task = cat?.tasks.find(t => t.id === p.task_id);
                const d = task?.details?.[0];
                return <tr key={p.id} className="border-b border-[#E0BBE4]/15 hover:bg-white/20">
      ...
      {/* Modal centrado con z-index alto y fondo oscuro */}
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="serif text-2xl text-[#4a3a5c]">Registrar pago</h3>
